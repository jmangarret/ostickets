...
<div class="clear"></div>
<h2 style="padding:10px 0 5px 0; font-size:11pt;margin-bottom:10px;color:#000;background-color:#F4FAFF"><big><big><?php echo Format::htmlchars($ticket->getSubject()); ?></big></big></h2>
<?php
...
?>
            ...
            <tr>
                ...
                <td>
                    ...
                    ?>
                    <label><input type="radio" name="signature" value="none"> <?php echo __('None');?></label>
                    <?php
...
?>
            <tr>
                <td width="120">
                    <label><strong>Estado del Localizador</strong></label>
                </td>
                <td>
                    <select name="estado_localizador" id="estado_localizador">
                        <option value=""> — Ninguno — </option>
                        <?php

                            if($loc == "Anulado" || $_POST["estado_localizador"] == "Anulado")
                                echo '<option value="Anulado" selected>Anulado</option>';
                            else
                                echo '<option value="Anulado">Anulado</option>';

                            if($loc == "Emitido" || $_POST["estado_localizador"] == "Emitido")
                                echo '<option value="Emitido" selected>Emitido</option>';
                            else
                                echo '<option value="Emitido">Emitido</option>';

                            if($loc == "Itinerario Cancelado" || $_POST["estado_localizador"] == "Itinerario Cancelado")
                                echo '<option value="Itinerario Cancelado" selected>Itinerario Cancelado</option>';
                            else
                                echo '<option value="Itinerario Cancelado">Itinerario Cancelado</option>';

                            if($loc == "Localizador no Valido" || $_POST["estado_localizador"] == "Localizador no Valido")
                                echo '<option value="Localizador no Valido" selected>Localizador no Valido</option>';
                            else
                                echo '<option value="Localizador no Valido">Localizador no Valido</option>';

                            if($loc == "Reembolsado" || $_POST["estado_localizador"] == "Reembolsado")
                                echo '<option value="Reembolsado" selected>Reembolsado</option>';
                            else
                                echo '<option value="Reembolsado">Reembolsado</option>';

                            if($loc == "Reemitido" || $_POST["estado_localizador"] == "Reemitido")
                                echo '<option value="Reemitido" selected>Reemitido</option>';
                            else
                                echo '<option value="Reemitido">Reemitido</option>';

                            if($loc == "Ticket Sellado" || $_POST["estado_localizador"] == "Ticket Sellado")
                                echo '<option value="Ticket Sellado" selected>Ticket Sellado</option>';
                            else
                                echo '<option value="Ticket Sellado">Ticket Sellado</option>';
...
<script type="text/javascript">
    ...
    $("#cannedResp").change(function(){
        if($("#cannedResp").val() == 4)
            $("#estado_localizador").val("Anulado");

        if($("#cannedResp").val() == 3)
            $("#estado_localizador").val("Emitido");

        if($("#cannedResp").val() == 6)
            $("#estado_localizador").val("Itinerario Cancelado");

        if($("#cannedResp").val() == 7)
            $("#estado_localizador").val("Reembolsado");

        if($("#cannedResp").val() == 5)
            $("#estado_localizador").val("Reemitido");

        if($("#cannedResp").val() == 8)
            $("#estado_localizador").val("Ticket Sellado");

        if($("#cannedResp").val() == 9)
            $("#estado_localizador").val("Localizador no Valido");

    });

    $("#reply").submit(function(){
        if($("#response").val() == "")
            alert("Debe ingresar texto en la respuesta");
